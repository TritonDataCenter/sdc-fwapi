...
fakeCNAPIclient.prototype._handleFwDel =
  ...
  if (!server.hasOwnProperty('rules')) {
    return callback(
      new VError('/fw/del: server "%s" does not have any rules', serverUUID));
  }
  ...
  if (notFound.length !== 0) {
    return callback(
      new VError('/fw/del: server "%s" does not have rules: %s', serverUUID,
        notFound));
  }
...
fakeCNAPIclient.prototype._handleFwUpdate =
  ...
  if (!server.hasOwnProperty('rules')) {
    return callback(
      new VError('/fw/update: server "%s" does not have any rules', serverUUID));
  }
