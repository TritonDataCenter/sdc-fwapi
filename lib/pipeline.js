 ...
 * with (state, cb) as its arguments. state contains the output of previous steps
 * of the pipeline, keyed by function name.
 */
function pipeline(opts, callback) {
  var state = {};
  var funcs = [];

  opts['funcs'].forEach(function (f, i) {
    funcs.push(function (_, cb) {
      var name = f.name || 'func' + i;

      f(state, function (e, r) {
        state[name] = r;
        return cb(e, r);
      });
    });
