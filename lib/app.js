...
function waitForWorkflowInit(wfapi, log, callback) {
  ...
  function initWorkFlowRetry() {
    log.debug('Initializing workflows: attempt %d', att);
    wfapi.initWorkflows(function (err) {
      ...
      log.error(err, 'Error loading workflows (attempt: %d): retrying', att);
      att++;
...
function createWorkflowClient(opts, callback) {
  ...
  fs.readdir(conf.path, function (err, files) {
    ...
    return waitForWorkflowInit(new WFAPI(conf), opts.log, callback);
  });
}
