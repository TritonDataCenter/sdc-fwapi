...
function usage() {
  var text = [
    ...
    'list',
    'add -f <filename>',
    'get <rule uuid> owner_uuid=<user uuid>',
    'update <rule uuid> -f <filename>',
    'delete <rule uuid> owner_uuid=<user uuid>',
    'resolve -f <filename>',
    'ping',
    'log',
    'lastlog',
...
function standardHandler(err, obj, req, res) {
  ...
  waitForWorkflowJob({ uuid: obj.job_uuid, url: CONFIG.wfapi.url },
    function (err, res2) {
      if (err) {
        if (err.code == 'timeout') {
          return console.log(cli.json(res2.chain_results));
        }
