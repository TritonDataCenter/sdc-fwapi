...
function ruleText(raw) {
  ...
  forEachTarget(raw, function(dir, type, name, arr) {
    for (var i in arr) {
      var txt = sprintf("%s %s", type, arr[i]);
      if (type == "wildcard") {
        txt = arr[i];
      }
...
function FwRule(data) {
  ...
  if (data.objectclass) {  // from UFDS
    ...
    this.dn = FwRule.dn(raw.fwrule, data.user, data.datacenter);
  }
...
FwRule.dn = function (fwrule, user, dc) {
  if (user == DC && dc) {
    return sprintf("fwrule=%s, ou=fwrules, datacenter=%s, o=smartdc", fwrule, dc);
  }
  return sprintf(
...
FwRule.dnFromRequest = function (req) {
  var fwrule = req.uriParams.fwrule;
  FwRule.validateUUID(fwrule);
  return FwRule.dn(fwrule, req.params.user, req._datacenter);
}

FwRule.parentDnFromRequest = function (req) {
  return FwRule.parentDn(req.params.user, req._datacenter);
}
