...
function ruleText(raw) {
  ...
  forEachTarget(raw, function(dir, type, name, val) {
    targets[dir] += sprintf("%s %s%s %s%s ",
      dir.toUpperCase(),
      val.length > 1 ? '( ' : '',
      type,
      val.join(' OR ' + type + ' '),
  ...
  return sprintf("%s%s%s %s PORT %s%s%s",
      targets.from,
      targets.to,
...
function FwRule(data) {
  ...
  if (data.objectclass) {  // from UFDS
    ...
    //var parsed = FwRule.parseDn(data.dn)
    ...
    this.dn = FwRule.dn(data.user, raw.fwuuid);
  }

  this.raw = raw;

  //Probe.validateUUID(raw.amonprobe);
...
FwRule.dn = function (user, fwuuid) {
  return sprintf(
...
FwRule.dnFromRequest = function (req) {
  ...
  return FwRule.dn(req.uriParams.user, fwuuid);
};
