...
/**
 * Returns the workflow parameters for the given rule and action
 */
function workflowParams(rule, action, override) {
    var params = {
        dn: rule.dn,
        target: action + '-' + rule.uuid,
        fwapiTags: {},
        task: action,
        rule: rule.serialize(),
    ...
    rule.tags.forEach(function (t) {
        if (typeof (t) === 'object') {
            params.fwapiTags[t[0]] = t[1];
        } else {
            params.fwapiTags[t] = true;
        }
    });

    if (rule.owner_uuid) {
        params.fwapiRuleOwner = rule.owner_uuid;
    }

    if (rule.allVMs) {
        params.fwapiAllVMs = true;
    }
...
function createRule(req, res, next) {
    ...
    var params = workflowParams(rule, 'add');

    req._wfapi.createJob('fw-add', params, function (err, job) {
        if (err) {
            return next(err);
        }

        req.log.debug({ params: params, job: job },
            'Add job "%s" queued for rule "%s"', job.uuid, rule.uuid);
...
function updateRule(req, res, next) {
    ...
    var params = workflowParams(newRule, 'update', {
        fwapiTags: tags,
        fwapiVMs: Object.keys(vms)
    });

    req._wfapi.createJob('fw-update', params, function (err, job) {
        if (err) {
            return next(err);
        }

        req.log.debug({ params: params, job: job },
            'Update job "%s" queued for rule "%s"', job.uuid, newRule.uuid);
...
function deleteRule(req, res, next) {
    var params = workflowParams(req._rule, 'del');
    params.rule_uuid = req._rule.uuid;

    req._wfapi.createJob('fw-del', params, function (err, job) {
        if (err) {
            return next(err);
        }
