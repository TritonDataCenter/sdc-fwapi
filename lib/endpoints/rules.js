...
/**
 * Restify 'before' handler for getting an existing rule from UFDS and
 * making sure the user is allowed to modify it. Stores the rule in
 * req._rule, and its dn in req._dn on success.
 */
function validateRuleOwner(req, res, next) {
  var dn;
  ...
  ufdsmodel.modelGet(req._app, Rule, dn, req.log, function (err, rule) {
    ...
    if (req.params.hasOwnProperty('owner_uuid')
      ...
      return next(new restify.ForbiddenError('owner_uuid does not match'));
      // XXX: uncomment when we move to restify 2.0
...
function register(server, before) {
  var matchingOwner = before.concat(validateRuleOwner);
