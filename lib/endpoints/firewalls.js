...
function ufdsGetMachineRules(req, uuid, machineData, callback) {
  ...
  var filter = [ '(&', '(enabled=true)',
    '(|', '(frommachine=' + uuid + ')', '(tomachine=' + uuid + ')',
          '(fromwildcard=all)', '(towildcard=all)'];
...
/**
 * Filter the output of machineData
 */
function filterMachineData(req, data) {
  var list = req.params.list;
  var uuid = req.uriParams.machineuuid;
  var log = req.log;
  log.trace("[%s] filterMachineData: start", uuid);
  ...
  var toList = {};
  var validList = { 'tags': 1, 'tag_ips': 1, 'machines': 1, 'rules': 1, 'all': 1 };

  list.split(/\s*,\s*/).forEach(function(el) {
    if (!validList.hasOwnProperty(el)) {
      return;
    }
    toList[el] = 1;
  });

  if (toList.hasOwnProperty('all')) {
    log.trace("[%s] filterMachineData: filter=all", uuid);
    return data;
  }

  var filtered = Object.keys(toList).reduce(function(ret, x) {
    ret[x] = data[x];
    return ret;
  }, {});

  log.trace("[%s] filterMachineData: filter=%s", uuid, JSON.stringify(toList));
  return filtered;
}
...
function getFirewall(req, res, next) {
  ...
  async.auto(tasks, function assemblePayload(err, results) {
    req.log.trace('machineData: assemblePayload entered');
    ...
    var filtered = filterMachineData(req, data);
...
function register(server, before) {
  server.get({ path: '/firewalls/:machineuuid', name: 'getFirewall' },
      before, getFirewall);
}
