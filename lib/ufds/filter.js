...
function ruleFilter(opts) {
  ...
  if (params.hasOwnProperty('tag')) {
    log.debug(params.tag, 'before');
    if (typeof (params.tag === 'string')) {
      params.tag = arrayify(params.tag);
    }

    if (util.isArray(params.tag)) {
      params.tag = params.tag.reduce(function (acc, t) {
        acc[t] = true;
        return acc;
      }, {});
    }

    if (Object.keys(params.tag) !== 0) {
      beginOR();
      ...
      for (var t in params.tag) {
        ...
        if (params.tag[t] !== true) {
          // Looking for key and value

          filter.push(util.format('(fromtag=%s=%s=*)', t, params.tag[t]));
          filter.push(util.format('(totag=%s=%s=*)', t, params.tag[t]));
        }
      }
  ...
  if (params.hasOwnProperty('owner_uuid')) {
    ...
    if (opts.operation !== 'OR') {
      // This is just another condition to add to the list of things to AND
      filter = ownerFilter.concat(filter);
    } else {
      // The owner filter is ANDed with the rest of the filter, so stick an
      // OR in front of them
      filter.unshift('(|');
      filter.push(')');
      filter = ownerFilter.concat(filter);
