...
function waitForWorkflowJob(opts, callback) {
    ...
    var wfapiClient = restify.createJsonClient(clientOpts);
    ...
    function interval() {
        return wfapiClient.get(endpoint, function (err, req, res, job) {
            ...
            if (opts.printStatus) {
                // Initial time through this loop: figure out the maximum
                // length of fields that we're going to print
                if (chainTotal === 0) {
                    var maxNameLen = 1;
                ...
                while (numPrinted < job.chain_results.length) {
                    ...
                    console.log(sprintf(outputLine, Math.floor(
                        (numPrinted / chainTotal) * 100), name, msg));
                    numPrinted++;
                }
            }
            ...
            if (job.execution == 'succeeded' || job.execution == 'failed') {
                ...
                if (job.execution == 'failed') {
                    return callback(new Error(msg), job);
                }
            ...
            if ((Date.now(0) - startTime) / 1000 > timeout) {
                ...
                var timeoutErr = new VError('job %s timed out', opts.uuid);
